...
{
	require_once get_template_directory().'/includes/vp_new/classes/MetaBox.php';//VP_FileSystem::instance()->resolve_path('includes', 'wpalchemy/MetaBox');
}
...
class VP_MetaBox_Alchemy1 extends WPAlchemy_MetaBox1
{
	...
	function _render_repeating_group($group)
	{
		...
		foreach ($group['groups'] as $g)
		{
			...
			$html .= '<div class="vp-wpa-group-heading"><a href="#" class="vp-wpa-group-title">' . $icon . $group['title'] . '</a><a href="#" class="dodelete vp-wpa-group-remove" title="'. __('Remove', SH_NAME) .'"><i class="fa fa-remove"></i> '. __('Remove', SH_NAME) .'</a></div>';
			$html .= '<div class="vp-controls' . ((!$is_first) ? ' vp-hide' : '') . '">';
